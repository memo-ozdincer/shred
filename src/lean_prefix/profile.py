...
def profile_replay_shard(
    ...
    *,
    ...
    eligible = units_total = reached_units = unreachable_units = 0
    ...
    def stop_client() -> None:
        nonlocal client, base_env, root_theorem, root_proof_state, since_restart
    ...
    def ensure_root(active: LeanRepl, env: int, theorem: str, statement: str) -> int:
        nonlocal root_theorem, root_proof_state, root_wall, root_cpu
        if root_theorem == theorem and root_proof_state is not None:
            return root_proof_state
        ...
        sorries = root.response.get("sorries", [])
        if len(sorries) != 1 or not isinstance(sorries[0].get("proofState"), int):
            raise ReplayProfileError(
                f"expected one root proof state for {theorem}, received {sorries!r}"
            )
        root_theorem = theorem
        root_proof_state = int(sorries[0]["proofState"])
        return root_proof_state
